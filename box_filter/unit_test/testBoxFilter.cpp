...
TEST(netTest, fast_boxfilter_v1) 
{
    std::vector<float> input;
    std::vector<float> output;
    ...
    for (int i = 0; i < loop; ++i) {
        ...
        tmp = double(duration.count()) * std::chrono::microseconds::period::num / 1000;
        avgTime += tmp;
        std::cout << "          [" << i << "]" << " Fast BoxFilfer V1 Cost time: " << tmp << "ms" << std::endl;
    }
    std::cout << "\n          Fast BoxFilfer V1 Average Cost time: " << avgTime / loop << "ms" << std::endl;
...
TEST(netTest, fast_boxfilter_v2_neon_asm_v1)
{
    ...
    for (int i = 0; i < loop; ++i) {
        auto startClock = std::chrono::system_clock::now();
        boxFilter.fastFilterV2NeonAsmV1(&input[0], radius, height, width, &output[0]);
        auto endClock = std::chrono::system_clock::now();
        auto duration = std::chrono::duration_cast<std::chrono::microseconds>(endClock - startClock);
        tmp = double(duration.count()) * std::chrono::microseconds::period::num / 1000;
        avgTime += tmp;
        std::cout << "          [" << i << "]" << " Fast BoxFilfer V2 Neon Asm V1 Cost time: " << tmp << "ms" << std::endl;
    }
    std::cout << "\n          Fast BoxFilfer V2 Neon Asm V1 Average Cost time: " << avgTime / loop << "ms" << std::endl;
