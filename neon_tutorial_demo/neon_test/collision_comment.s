...
_Z12does_collideR6circleS0_:            // @_Z12does_collideR6circleS0_
	.cfi_startproc
// %bb.0:
	sub	sp, sp, #32    // allcate stack
	.cfi_def_cfa_offset 32
	str	x0, [sp, #24]  // store c1 to stack
	str	x1, [sp, #16]  // store c2 to stack
	ldr	x8, [sp, #24]  // load c1 to x8
	ldr	s0, [x8, #4]   // load c1.x to s0
	ldr	x8, [sp, #16]  // load c2 to x8
	ldr	s1, [x8, #4]   // load c2.x to s1
	fsub	s0, s0, s1 // cal c1.x - c2.x to s0
	str	s0, [sp, #12]  // store result to sp + 12
	ldr	x8, [sp, #24]  // load c1 to x8
	ldr	s0, [x8, #8]   // load c1.y to s0
	ldr	x8, [sp, #16]
	ldr	s1, [x8, #8]
	fsub	s0, s0, s1 // cal c1.y - c2.y to s1
	str	s0, [sp, #8]   
	ldr	s0, [sp, #12]  // load c1.x - c2.x to s0 
	ldr	s1, [sp, #12]  // load c1.x - c2.x to s1
	ldr	s2, [sp, #8]   // load c1.y - c2.y to s2
	ldr	s3, [sp, #8]
	fmul	s2, s2, s3  // Calculate (c1.y - c2.y)2 to s2
	fmadd	s0, s0, s1, s2 // mac, s0 * s1 + s2, i.e. calculate deltas_squared
	str	s0, [sp, #4]
	ldr	x8, [sp, #24]
	ldr	s0, [x8]
	ldr	x8, [sp, #16]
...
_Z17does_collide_neonRK14circle_alignedS1_: // @_Z17does_collide_neonRK14circle_alignedS1_
	.cfi_startproc
// %bb.0:
	sub	sp, sp, #160   // allocate stack
	.cfi_def_cfa_offset 160
	add	x8, sp, #136   // x8 to store the ret address 
	str	x0, [sp, #88]  // c1 -> sp + 88
	str	x1, [sp, #80]  // c2 -> sp + 80
	ldr	x9, [sp, #88]  // c1 -> x9
	ldr	d0, [x9]       // c1 -> d0
	str	d0, [sp, #64]  // d0 -> sp + 64
	ldr	d0, [sp, #64]  // sp + 64 -> d0
	str	d0, [sp, #56]  // d0 -> sp + 56
	ldr	d0, [sp, #56]
	str	d0, [sp, #72]  
	ldr	x9, [sp, #80]  
	ldr	d0, [x9]	   // c2 -> d0
	...
	ldr	d0, [x8]
	ldr	d1, [sp, #128]
	fmul	v0.2s, v0.2s, v1.2s // Square both deltas at the same time
	str	d0, [sp, #120]
	ldr	d0, [sp, #120]
	str	d0, [sp, #16]
	ldr	d0, [sp, #16]
	...
	str	s0, [sp, #148] // Code from here is scalar, same as before...
	ldr	w8, [sp, #148]
	str	w8, [sp, #12]
	ldr	x8, [sp, #88]
	ldr	s0, [x8, #8]
